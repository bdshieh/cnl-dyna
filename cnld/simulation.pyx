...
class CompensatingFixedStepSolver:
    ...
    def __init__(self, fir_t, fir, v_t, v, gaps, t_start, t_stop, fcomp, atol=1e-10, maxiter=5):
    ...
    def from_array_and_db(cls, array, refn, dbfile, v_t, v, t_start, t_stop, atol=1e-10, maxiter=5):
        ...
        return cls(fir_t, fir, v_t, v, gaps, t_start, t_stop, fcomp, atol, maxiter)
    ...
    def reset(self):
        ...
        x0 = np.zeros(self.npatch)
        self._displacement = [x0,]
        p0 = pressure_es2(self._voltagef(t_start), x0, self._fcomp)
        self._pressure = [p0,]
